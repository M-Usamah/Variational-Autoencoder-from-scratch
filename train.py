...
from VAC import VariationAutoEncoder
...
NUM_EPOCHS = 10
BATCH_SIZE = 32
LR_RATE = 4e-4  # Karpathy constant

# Dataset Loading
dataset = datasets.MNIST(root="dataset/", train=True, transform=transforms.ToTensor(), download=True)
train_loader = DataLoader(dataset=dataset, batch_size=BATCH_SIZE, shuffle=True)
model = VariationAutoEncoder(INPUT_DIM, H_DIM, Z_DIM).to(DEVICE)
...
for epoch in range(NUM_EPOCHS):
    ...
    for i, (x, _) in loop:
        ...
        # Backprop
        loss = reconstructed_loss + ki_div
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
